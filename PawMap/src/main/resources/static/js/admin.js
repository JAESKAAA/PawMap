...
/* 
	Dec 13th 07:44am by thomas lee
	위의 재석이 함수를 이용해 admin페이지에서 검색시 
	더 많은 자료가 검색하겠금 함수 modify했고 적용함 
	하기는 유저 및 병원 유저 검색 함수.
...
/* 
	Dec 13th 07:45am by thomas lee
	위의 재석이 함수를 이용해 admin페이지에서 보호소 검색시 
	더 많은 자료가 검색하겠금 함수 modify했고 적용함 
	하기는 유저 및 병원 유저 검색 함수.
...
function admin_shelter_search(){
 ...
 console.log(myParam);
	...
	if(shelterList.length == 0){
		console.log("if 내부 로그");
	console.log(shelterList.length);
	$.ajax({
		type : "GET",
		url : "/pawmap/getShelterByJson",
		dataType : "json",
		async : false,
		success : function(response){
			console.log(typeof(response));
			console.log(response.shelterList[2]);
			
		for(let i = 0; i <response.shelterList.length; i++){
			
			shelterList.push(response.shelterList[i]);
		}	
			},
		error : function(e){
			alert(e);
	...
	for(var i=0; i<shelterList.length; i++){
		if(shelterList[i].shelterName.indexOf(searchValue)!= -1 ){
			searchList.push(shelterList[i]);
		}
	}
	console.log(searchList);
let str=`<tr>
			<th class="admin-userseq">#</th>
			<th class="admin-username">보호소 이름</th>
			<th class="admin-address">보호소 주소</th>
			<th class="admin-usertelnum">연락처</th>
...
if(searchValue==""){
 let pagingList= shelterList.slice(pageNum,pageNum+11);
 console.log(pagingList);
 for(var i=0; i<pagingList.length; i++){
  str+=
	  `<tr>
		  <td class="admin-userseq">${pagingList[i].shelterSeq }</td>
		  <td class="admin-username">${pagingList[i].shelterName }</td>
		  <td class="admin-address">${pagingList[i].shelterAddress }</td>	
		  <td class="admin-usertelnum">${pagingList[i].shelterTel }</td>
		  <td class="admin-comnum">${pagingList[i].comNum }</td>
		  <td class="admin-useractionbutton"><button class="adminpagebuttonformanage adminbuttonmargin-right" type="button" onclick="location.href='/pawmap/admin/updateShelterForm?shelterSeq=${pagingList[i].shelterSeq}'">수정</button>
                                                          <button class="adminpagebuttonformanage adminbuttonmargin-left" type="button" id="${pagingList[i].shelterSeq}" onclick="fnDeleteShelter(this);">삭제</button></td>
	  </tr>`;
 console.log(pagingList[i].shelterSeq);
 };
} else{
 for(var i=0; i<searchList.length; i++){
	 str+=
		  `<tr>
	 			<td class="admin-userseq">${searchList[i].shelterSeq }</td>
	 			<td class="admin-username">${searchList[i].shelterName }</td>
	 			<td class="admin-address">${searchList[i].shelterAddress }</td>	
	 			<td class="admin-usertelnum">${searchList[i].shelterTel }</td>
			 	<td class="admin-comnum">${searchList[i].comNum }</td>
				 <td class="admin-useractionbutton"><button class="adminpagebuttonformanage adminbuttonmargin-right" type="button" onclick="location.href='/pawmap/admin/updateShelterForm?shelterSeq=${searchList[i].shelterSeq}'">수정</button>
													 <button class="adminpagebuttonformanage adminbuttonmargin-left" type="button" id="${searchList[i].shelterSeq}" onclick="fnDeleteShelter(this);">삭제</button></td>
		   </tr>`;
}
$("#table_shelter_admin").html(str);
	}
}
