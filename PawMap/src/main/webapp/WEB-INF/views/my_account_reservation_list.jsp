        ...
        <tr>
            ...
            <c:choose>
                <c:when test="${empty myResList.reservation_status }">
                    <td style="color: gray;"><button class="btnHos" type="button" onclick='location.href="/pawmap/showMyMedicalRecord?comNum=${myResList.com_num}&reservationDate=${myResList.reservation_date}&scheduleTime=${myResList.schedule_time}&userId=${principal.user.userId}"' disabled>진단서 보기</button></td>
                </c:when>
                <c:otherwise>
            ...
            <c:choose>
                <c:when test="${myResList.reservation_date <= sysYear || !empty myResList.reservation_status}">
                    <td><button class="btncan" disabled>예약취소</button></td>
                </c:when>
                <c:otherwise>
